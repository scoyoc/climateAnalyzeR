...
round_with_zeros <- function(x, d = 1) {
  as.numeric(format(round(x, digits = d), nsmall = d))
}
...
# Isolate month
my_mth <- dplyr::filter(mth_dat, water_yr == my_year) %>%
  dplyr::ungroup() %>%
...
This document summarizes current temperature and precipitation anomalies relative to 30-year (1981-2010) averages for `r my_station$name` during the `r my_year` water year (October `r my_year - 1` through September `r my_year`) through `r month.name[match(my_mth$month_name, month.abb)]` `r ifelse(my_mth$month_name %in% c('Oct', 'Nov', 'Dec'), my_year - 1, my_year)`.
...
# Text for water year temperature ranking.
temp_rank_text <- if(temp_text == "error"){
  glue::glue("There are too many missing records to meaningfully summarize temperature data from this station for {month.name[match(my_mth$month_name, month.abb)]} {my_mth$year}.")
...
# Text for monthly temperature
temp_mth_text <- glue::glue("The average temperature for {month.name[match(my_mth$month_name, month.abb)]} was {round_with_zeros(my_mth_temp$value)}°F which is {abs(round_with_zeros(my_mth_temp$departure))}°F {ifelse(my_mth_temp$departure > 0, 'above', 'below')} the 30-year average ({round_with_zeros(my_mth_temp$norms_30yr)}°F).")
...
cat(glue::glue("There are too many missing records to meaningfully summarize temperature data from this station for water year {my_year}."),
...
# Text for water year ranking
prcp_rank_text <- if(prcp_text == "error"){
  glue::glue("There are too many missing records to meaningfully summarize precipitation data from this station for water year {month.name[match(my_mth$month_name, month.abb)]} {my_mth$year}.")
} else if(prcp_text == "dry" & text_refs$prcp_driest == 1){
...
} else if(prcp_text == "wet" & text_refs$prcp_wettest == 1){
...
# Text for water year relative to data set
prcp_annual_text <- glue::glue("Accumulated precipitation is {round_with_zeros(my_mth_prcp$prcp_accum)} inches so far this water year which is  {round_with_zeros(my_mth_prcp$accum_depart)} inches {ifelse(my_mth_prcp$accum_depart < 0, 'below', 'above')} the 30-year average ({round_with_zeros(my_mth_prcp$prcp_accum_30yr)} inches).")

# Text for precipitation trend
prcp_trend_text <- glue::glue("This summary suggests that precipitation is {ifelse(prcp_lm > 0, 'increasing', 'decreasing')} at a rate of {abs(round_with_zeros(prcp_lm * 10, d = 2))} inches per decade (Figure 2A).")

# Text about condition
# TODO: Write condition for no months == 0
