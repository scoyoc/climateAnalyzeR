...
date: "`r format(as.Date(Sys.Date(), format = '%Y-%m-%d'), '%d %B, %Y')`"
output:
  pdf_document: 
    fig_caption: yes
...
tmax_exceed <- filter(mth_depart, waterYr == water_yr & TMAX_depart > 0)
tmin_exceed <- filter(mth_depart, waterYr == water_yr & TMIN_depart > 0)
prcp_above <- filter(mth_depart, waterYr == water_yr & PRCP_pctavg > 100)
prcp_below <- filter(mth_depart, waterYr == water_yr & PRCP_pctavg < 100)
text_refs <- park_unit %>%
...
t1b <- line_figure(dat = filter(t_dat, Element == "TMAX"), 
                   ...
                   wy_xvar = filter(t_dat, Element == "TMAX" & 
                                      waterYr == water_yr)$mthName,
                   wy_yvar = filter(t_dat, Element == "TMAX" & 
                                      waterYr == water_yr)$Departure,
                   line_color = "red3", 
