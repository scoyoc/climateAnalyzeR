---
title: "climateAnalyzeR Vignette"
author: "Matt Van Soyoc"
format: pdf
---
...
This vignette demonstrates how to use the various functions in the `climateAnalyzeR` package to import and visualize temperature and precipitation data from [Global Historical Climatology Network](https://www.ncei.noaa.gov/products/land-based-station/global-historical-climatology-network-daily) (GHCN) Co-op weather stations.

This package can also produce brief water year (Oct-Sep) and annual (Jan-Dec) climate reports using temperature and precipitation data from GHCN Co-op stations using RMarkdown.
However, this function is currently broken and I'm working on fixing it.
I will include an example here once it's fixed.
...
```{r map, fig.width=5, fig.height=6, message=FALSE, warning=FALSE}
# Get base tiles
...
knitr::include_graphics(
  file.path(here::here(), '/vignettes/url_screenshot.png')
  )
```
...
```{r ann_tmax, message=FALSE, warning=FALSE, fig.width=7, fig.height=3.5}
...
```{r ann_tmin, message=FALSE, warning=FALSE, fig.width=7, fig.height=3.5}
...
```{r ann_prcp, message=FALSE, warning=FALSE, fig.width=7, fig.height=3.5}
# Precipitation (PRCP) ----
...
```{r monthly_figures, fig.width=7, fig.height=9, message=FALSE, warning=FALSE}
...
```{r daily_tmax, message=FALSE, warning=FALSE, fig.width=7, fig.height=3.5}
...
```{r departure_figures, fig.width=7, fig.height=9, message=FALSE, warning=FALSE}
plot_title <- cowplot::ggdraw() +
