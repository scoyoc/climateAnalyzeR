...
#-- ToDo's
#TODO: Create text "chunks", then insert them into text sections to reduce redundancy
#TODO: Make rounding statements objects to reduce redundancy

#-- Packages
# devtools::install_github("scoyoc/climateAnalyzeR")
library("climateAnalyzeR")
# Note: See utils.R to see code for:
# number_contraction()
# glue_mths
...
number_contraction <- function(x) {
  if (is.na(x)){
>>>>>>> 7a6a09718de1b0d7f81940b04e345eff4263df0f
    y = FALSE
...
<<<<<<< HEAD
my_year <- 2020 # Throws an NA when ranking annual data @ ISKY
=======
my_year <- 2020 # Throws an NA when ranking annual data
>>>>>>> 7a6a09718de1b0d7f81940b04e345eff4263df0f
# my_year <- 1973 # Coldest @ ISKY
# my_year <- 2010 # Coldest @ ARCH
# my_year <- 1978 # 10th coldest @ ISKY
# my_year <- 1988 # 10th coldest @ ARCH
# my_year <- 2018 # Warmest
# my_year <- 1978 # 10th warmest @ ISKY
# my_year <- 2016 # 10th warmest @ ARCH
# my_year <- 1984 # wettest
# my_year <- 1980 # 10th wettest
# my_year <- 1977 # driest @ ISKY
# my_year <- 2021 # driest @ ARCH
# my_year <- 2000 # 10th driest

# (annual_dat %>%
#   dplyr::filter(temp_warmest == 10))$year
# (annual_dat %>%
#   dplyr::filter(temp_coldest == 1))$year
# (annual_dat %>%
...
temp_rank_text <- if(temp_error == "annual"){
  ...
  } else if(text_refs$temp_coldest == 1){
...
=======
```{r Print_Temp_Text, echo=FALSE, results='asis', eval=(temp_text %in% c("cool", "avg", "warm"))}
>>>>>>> 7a6a09718de1b0d7f81940b04e345eff4263df0f
...
check_data_text <- "Examine annual and monthly data sets on ClimateAnalyzer.org for more information."

error_trend_text <- glue::glue("However, this summary suggests that temperatures are {ifelse(temp_lm > 0, 'increasing', 'decreasing')} {abs(round_with_zeros(temp_lm * 10))}°F per decade (Figure 1A).")
...
cat(
  ...
  # Sentence 2. Annual
  glue::glue("Total accumulated precipitation was {text_refs$prcp_wy} inches and was {format(round(abs(text_refs$prcp_wy - annual_30yr$PRCP), digits = 1), nsmall = 1)} inches {ifelse(text_refs$prcp_wy - annual_30yr$PRCP < 0, 'below', 'above')} the 30-year average ({format(round(annual_30yr$PRCP, digits = 1), nsmall = 1)} inches)."), 
  # Sentence 3. Trend
  glue::glue("This summary suggests that precipitation is {ifelse(prcp_lm > 0, 'increasing', 'decreasing')} at a rate of {abs(as.numeric(format(round(prcp_lm * 10, digits = 1), nsmall = 1)))} inches per decade (Figure 2A)."), 
  # Sentence 4. Monthly
  if(prcp_text == "dry"){
    glue::glue("{paste(paste(prcp_above[1:length(prcp_above) - 1], collapse = ', '), prcp_above[length(prcp_above)], sep = ', and ')} received above average precipitation (Figure 2B) but were not enough to compensate for the {length(prcp_below)} months that were below average ({paste(paste(prcp_below[1:length(prcp_below)-1], collapse = ', '), prcp_below[length(prcp_below)], sep = ', and ')}; Figure 2C).")
    } else if (prcp_text == "avg"){
      glue::glue("{paste(paste(prcp_above[1:length(prcp_above)-1], collapse = ', '), prcp_above[length(prcp_above)], sep = ', and ')} received above average precipitation (Figure 2B) and {paste(paste(prcp_below[1:length(prcp_below)-1], collapse = ', '), prcp_below[length(prcp_below)], sep = ', and ')} received below average precipitation (Figure 2C).")
      } else if (prcp_text == "wet"){
        glue::glue("{paste(paste(prcp_above[1:length(prcp_above)-1], collapse = ', '), prcp_above[length(prcp_above)], sep = ', and ')} received above average precipitation (Figure 2B) and {paste(paste(prcp_below[1:length(prcp_below)-1], collapse = ', '), prcp_below[length(prcp_below)], sep = ', and ')} received below average precipitation (Figure 2C).")
  }
)
...
=======
  # Sentence 1. Intro
  glue::glue("There are too many missing records to meaningfully summarize data from this station for water year {my_year}."),
  # Sentence 2. Check data
  ("Examine annual and monthly data sets on ClimateAnalyzer.org for more information."),
  # Sentence 3. Trend
  glue::glue("However, this summary suggests that precipitation is {ifelse(prcp_lm > 0, 'increasing', 'decreasing')} by {abs(as.numeric(format(round(prcp_lm * 10, digits = 1), nsmall = 1)))} inches per decade (Figure 1A).")
  )}
>>>>>>> 7a6a09718de1b0d7f81940b04e345eff4263df0f
