...
# my_year <- 1993 # 10th coldest
# my_year <- 2018 # Warmest
# my_year <- 2007 # 10th warmest
# my_year <- 1984 # wettest
# my_year <- 2015 # 10th wettest
# my_year <- 1977 # driest
# my_year <- 1970 # 10th driest
...
# annual_dat %>%
#   dplyr::select(year, temp_warmest, prcp_driest)
# 
# (annual_dat %>%
#   dplyr::filter(temp_warmest == min(temp_warmest, na.rm = T) + 10))$year
...
>>>>>>> 44d99b699d1e06d45897b886f709e23501123b8d
...
=======
  prcp_error <- tibble::tibble(annual = ifelse(is.na(error_eval$prcp_wy), 
                               ...
                               month = ifelse(!is.na(error_eval$prcp_mth_na), 
                                              TRUE, FALSE)) %>%
    dplyr::mutate(both = ifelse((annual == TRUE & month == TRUE), 
                                TRUE, FALSE))
...
# TODO: push to Github and reinstall
library(ggmap)
...
my_map <- ggmap::get_map(location = my_box, maptype = "hybrid", source = "google", 
...
cat(
  # Sentence 1
  if (text_refs$temp_warmest == max(annual_dat$temp_warmest, na.rm = TRUE)){
    paste0("Water year ", my_year, " was the coldest year on record for ", my_station$name, ".")
    } else if (temp_text == "cool"){
    paste0("Water year ", my_year, " was the ",  number_contraction(max(annual_dat$temp_warmest, na.rm = TRUE) - text_refs$temp_warmest), " coolest water year in the ", text_refs$timespan, "-year record for ", my_station$name, ".")
...
cat(
  ...
  } else if (text_refs$prcp_driest == max(annual_dat$prcp_driest, na.rm=TRUE)){
    paste0("Water year ", my_year, " was the wettest year on record for ", my_station$name, ".")
  } else (paste0("Water year ", my_year, " was the ", number_contraction(max(annual_dat$prcp_driest, na.rm=TRUE) - text_refs$prcp_driest), " wettest on record for ", station_name, ".")), 
