...
# Inadequate annual data -- throws an NA when ranking annual data 
...
# TODO: Evaluate if error is in annual or monthly data set. Then write conditional text.
# TODO: Test missing month threshold for annual ranking.
# 1. Pull annual data and rank.
# 2. Pull monthly data and count NA's for temp/prcp data.
# 3. Compare annual ranking to # of monthly NA's. Filter rows of monthly NA's > 1.
# 4. write lappy to cycle through co-op stations.

error_eval <- tibble::tibble(annual_dat[, c("year", "temp_warmest", 
                                            "tmax_warmest", "tmin_warmest", 
                                            "prcp_driest")]) %>%
  dplyr::left_join(mth_depart %>% 
                     dplyr::filter(is.na(tmax_depart)) %>% 
                     dplyr::group_by(year) %>%
                     dplyr::count()) %>%
  dplyr::rename("monthly.tmax.na" = "n") %>%
  dplyr::left_join(mth_depart %>% 
                     dplyr::filter(is.na(tmin_depart)) %>% 
                     dplyr::group_by(year) %>%
                     dplyr::count()) %>%  
  dplyr::rename("monthly.tmin.na" = "n") %>%
  dplyr::left_join(mth_depart %>% 
                     dplyr::filter(is.na(prcp_pctavg)) %>% 
                     dplyr::group_by(year) %>%
                     dplyr::count()) %>%
  dplyr::rename("monthly.prcp.na" = "n") %>%
  dplyr:: mutate("total" = monthly.tmax.na + monthly.tmin.na + monthly.prcp.na) %>%
...
my_box <- make_bbox(lon = extent$lon, lat = extent$lat, f = 0.1)
my_map <- ggmap::get_map(location = my_box, maptype = "satellite", source = "osm", 
                  zoom = 13, force = TRUE, messaging = TRUE)
my_map <- ggmap::ggmap(my_map) + 
  ggplot2::geom_point(data = my_station, aes(x = lon, y = lat), 
                      color = "red", size = 4) +
...
```{r Insufficient_Temp, echo=FALSE, results='asis', eval=(temp_text == TRUE)}
# Text is printed in PDF if is.na(text_refs) == TRUE
cat(
  # Sentence 1
  paste0("There are too many missing records to meaningfully summarize data from this station for water year ", my_year, ";"), 

  # Sentence 2
  ("Examine annual and monthly tabular data sets on ClimateAnalyzer.org for more information."),
...
cat("Error: temperature data could not be evaluated. Examine tabular data set on ClimateAnalyzer.org.")
```
...
```{r Insufficient_PRCP, echo=FALSE, results='asis', eval=(prcp_text == TRUE)}
cat(
  # Sentence 1
  paste0("There are too many missing records to meaningfully summarize data from this station for water year ", my_year, ";"), 
